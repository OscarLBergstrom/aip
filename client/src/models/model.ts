...
export default class HaipModel {
  ...
  handleLogin = async () => {
    ...
    try {
      ...
      const apiUrl =
        ...
        `/api/login?challenge=${encodeURIComponent(challenge)}`;
  ...
  /**
   * Insert the created playlist into the database.
   */
  ...
  getPlaylists = async () => {
    try {
      const data = await useFetch({
        url:
          (process.env.REACT_APP_URL || "http://localhost:3001") +
          `/api/getplaylists?token=${encodeURIComponent(
            this.user.token
  ...
  getSearchResult = async (track: string, artist: string) => {
    try {
      const data = await useFetch({
        url:
          (process.env.REACT_APP_URL || "http://localhost:3001") +
          `/api/search?token=${encodeURIComponent(
            this.user.token
  ...
  getUserProfile = async () => {
    try {
      const data = await useFetch({
        url:
          (process.env.REACT_APP_URL || "http://localhost:3001") +
          `/api/profile?token=${encodeURIComponent(this.user.token)}`,
        method: "GET" as Method,
      });
